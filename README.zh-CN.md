...
以上原则定义了项目的信任模型。0.1.1 版本只在一个范围明确的 Python 编码任务闭环中
...
- Agent 调用边界把异常和非法返回值转换为结构化拒绝，而不是让控制器失控；
- ChallengePolicy 限制 Critic 提议的检查类型、数量、标识符和负载大小；
- 跨越不可信组件边界时使用规范化独立快照，阻止 Agent 原地篡改控制器状态；
- 失败收据也必须验证完整性、绑定关系和签名，才能用于指导修复；
